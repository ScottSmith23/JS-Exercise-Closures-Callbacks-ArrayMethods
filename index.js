...
function processSum(numberList,callback) {
  let total = numberList.reduce((accumulator,sum)=>
...
function firstNamesAllCaps(runners) {
  const newArray = runners.map((runners) => 
...
function getRunnersByTShirtSize(runners,tShirtSize) {
  const newArray = runners.filter((elements)=>{
  return elements.shirt_size === tShirtSize;});
  return newArray;
}
...
function tallyUpDonations(runners) {
  const sum = runners.reduce(function(accumulator, item){
